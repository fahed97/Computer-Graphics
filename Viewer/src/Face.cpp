...
void Face::FacePrint()const
{
	for (std::vector<int>::size_type vec = 0; vec != vertex_indices_.size(); vec++) {
		std::cout << "Vertex Index: " << vertex_indices_[vec];
		std::cout << " Normal Index: " << normal_indices_[vec];
		std::cout << " Texture Index: " << texture_indices_[vec]<<"\n";
	}

}
const glm::vec3 Face::getFaceColor() const
{
	return color;
}
const void Face::setFaceColor(glm::vec3& color)
{
	this->color = color;
}
